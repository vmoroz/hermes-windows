...
/*static*/ std::string NapiTestException::CoerceToString(
    napi_env env,
    napi_value value) {
  napi_value strValue;
...
/*static*/ std::string NapiTestException::ToString(
    napi_env env,
    napi_value value) {
  size_t valueSize{};
...
NapiTestContext::GetCommonScripts(std::string const &testJSPath) noexcept {
  std::map<std::string, TestScriptInfo, std::less<>> moduleScripts;
  moduleScripts.try_emplace(
      "assert",
      TestScriptInfo{
          ReadScriptText(testJSPath, "common/assert.js"),
          "common/assert.js",
          1});
  moduleScripts.try_emplace(
      "../../common",
      TestScriptInfo{
          ReadScriptText(testJSPath, "common/common.js"),
          "common/common.js",
          1});
...
napi_value NapiTestContext::GetModule(std::string const &moduleName) {
  ...
  if (moduleIt != m_modules.end()) {
    ...
    if (moduleName.find("@babel") == 0) {
      ...
      result = RunScript(
          GetJSModuleText(ReadScriptText(m_testJSPath, scriptFile)),
          scriptFile.c_str());
    } else if (
        moduleName.find("./") == 0 &&
...
std::string NapiTestContext::ReadScriptText(
